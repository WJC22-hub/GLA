...
def params():
    ...
    parser.add_argument('--lora', action='store_true',default=False, help='Perform LoRA finetuning, recommend if '
    ...
    parser.add_argument('--model-name', default='20251017-033019-backdoor-trans-200', type=str, help='The checkpoint to load from '
...
if __name__ == '__main__':
    ...
    model.load_state_dict(
        torch.load(os.path.join('multi_frame_results', config.model_name,
                                'latest_model.pth')))  # latest_model.pth
    ...
    # BLEU
    bleu_scorer = Bleu()
    bleu_score, bleu_scores = bleu_scorer.compute_score(gts_tokenize, res_tokenize)
    if type(bleu_score) == list:
        for i, score in enumerate(bleu_score):
            eval_results[f'Bleu_{i+1}'] = score
    else:
        eval_results['Bleu_1'] = bleu_score
    
    # METEOR
    meteor_scorer = Meteor()
    meteor_score, meteor_scores = meteor_scorer.compute_score(gts_tokenize, res_tokenize)
    eval_results['METEOR'] = meteor_score

    # ROUGE-L
    rouge_scorer = Rouge()
    rouge_score, rouge_scores = rouge_scorer.compute_score(gts_tokenize, res_tokenize)
    eval_results['ROUGE_L'] = rouge_score
